...
class StreamingASRProcessor:
    ...
    def setup_credentials(self):
        ...
        if os.path.exists(credentials_path):
            try:
                # Check if it's a mock credentials file
                with open(credentials_path, 'r') as f:
                    content = f.read()
                    if 'MOCK_PRIVATE_KEY_FOR_DEVELOPMENT' in content:
                        logger.warning("⚠️  Using mock credentials for development - Google Cloud API calls will fail")
                        logger.info("💡 For production, replace tts-key.json with real Google Cloud service account key")
    ...
    def process_audio_stream(self, audio_generator: Generator[bytes, None, None], 
        ...
        try:
            if self.mock_mode:
                logger.warning("⚠️  Running in mock mode - simulating ASR responses")
                # Simulate ASR processing in mock mode
                chunk_count = 0
                for audio_chunk in audio_generator:
                    if audio_chunk is None:
                        break
                    chunk_count += 1
                    logger.debug(f"Mock processing audio chunk {chunk_count}: {len(audio_chunk)} bytes")
                    
                    # Simulate interim and final results
                    if chunk_count % 3 == 0:  # Every 3rd chunk, send interim result
                        mock_response = {
                            "type": "transcript",
                            "text": f"Mock interim transcription chunk {chunk_count}",
                            "is_final": False,
                            "confidence": 0.8,
                            "language_code": streaming_config.config.language_code
                        }
                        asyncio.run_coroutine_threadsafe(
                            websocket.send(json.dumps(mock_response)), loop
                        )
                    
                    if chunk_count % 10 == 0:  # Every 10th chunk, send final result
                        mock_response = {
                            "type": "transcript",
                            "text": f"Mock final transcription for chunks 1-{chunk_count}",
                            "is_final": True,
                            "confidence": 0.9,
                            "language_code": streaming_config.config.language_code
                        }
                ...
                completion_response = {
                    ...
                    "message": "Mock ASR processing completed",
                    "total_chunks": chunk_count
                }
                asyncio.run_coroutine_threadsafe(
                    websocket.send(json.dumps(completion_response)), loop
                )
                return
